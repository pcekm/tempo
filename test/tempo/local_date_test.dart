...
void main() {
  group('Constructors and basic getters:', () {
    ...
    test('now()', () {
      ...
      //
      // If DateTime's time zone doesn't match, then at least some of those
      // hours will occur on the wrong day, and the test will fail.
      //
      // * If you're running this test in the easternmost islands of Kiribati,
      // hello! I apologize. This test won't be quite as effective, but it
      // should at least pass.
      defaultZoneId = 'Pacific/Kiritimati';
      for (var hour = 0; hour <= 23; hour++) {
        var d = withClock(
            Clock.fixed(DateTime.utc(2026, 1, 2, hour)
                .subtract(Duration(hours: 14)) // Kiritimati's UTC offset
                .toLocal()),
            () => LocalDate.now());
        expect(d, hasDate(2026, 1, 2));
      }
    });
  });
  ...
  group('replace()', () {
    test('year', () {
      expect(LocalDate(2023, 1, 2).replace(year: 1998), hasDate(1998, 1, 2));
    });

    test('month', () {
      expect(LocalDate(2023, 1, 2).replace(month: 11), hasDate(2023, 11, 2));
    });

    test('day', () {
      expect(LocalDate(2023, 1, 2).replace(day: 31), hasDate(2023, 1, 31));
    });
    ...
    test('day adjustment, shorter month', () {
      expect(LocalDate(2023, 1, 31).replace(month: 6), hasDate(2023, 6, 30));
    });

    test('day adjustment, leap year', () {
      expect(LocalDate(2000, 2, 29).replace(year: 2001), hasDate(2001, 2, 28));
    });
  });
  ...
  group('conversions', () {
    ...
    test('inTimezone', () {
      final want = ZonedDateTime(2000, 1, 2);
  ...
  group('periodUntil():', () {
    ...
    test('golden file', () {
      ...
      var file = File('test/tempo/local_date_period_until_testcases.txt');
